...
public class VisualRightRefractionAction {
    ...
    //---------------------------------------------
    private XYDataset createDataset(ArrayList<Double> rightRefraction, ArrayList<String> checkTimeList) {	//,int age
    	TimeSeriesCollection dataset = new TimeSeriesCollection();
    
    	TimeSeries timeSeries1 = new TimeSeries("右眼屈光度图",Day.class);
	    try{
	    	for(int i=0;i<rightRefraction.size();i++){
	    		String[] value=((String) checkTimeList.toArray()[i]).split("-");
	    		if(value.length==3){
	    			//年月日展示 2016-1-26
	    			timeSeries1.addOrUpdate(new Day(Integer.parseInt(value[2]),Integer.parseInt(value[1]), Integer.parseInt(value[0])) , (Double)rightRefraction.toArray()[i]);
	    		}
   ...
   public String rightRefraction() {
	   System.out.println("This is rightRefraction() of VisualRightRefractionAction!");
    	try {
...
//			System.out.println("用户"+u.getUsername()+"视觉档案的数量为："+list.size());
			...
			for (int i = 0; i < list.size(); i++) {
				if(list.get(i).getRefractionRightEyeAverages()!=null && !list.get(i).getRefractionRightEyeAverages().equals("")){
					try{
						temp_value=Double.valueOf(list.get(i).getRefractionRightEyeAverages());
						dt = df.parse(list.get(i).getCheckTime());
					}catch(Exception e){
						System.out.println("第"+i+"次循环，rightRefraction视力中有错误数据！");
						continue;
					}
					rightRefraction.add(temp_value);
					checkTimeList.add(df.format(dt));
				}
			}
			//改为时序图显示多条折线 2016-1-19
			this.chart = ChartFactory.createTimeSeriesChart("右眼屈光度","检测时间", "数值", createDataset(rightRefraction,checkTimeList), true,true, false);
