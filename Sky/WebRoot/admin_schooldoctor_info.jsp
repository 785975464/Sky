...
<body>
						...
						<table id="pageTable">
							<tr>
								<td><s:if test="%{pageBean.currentPage == 1}">首页      上一页</s:if>
									<s:else>
										<a href="schoolDocInfo!schoolDocInfo?page=1">首页</a>
										<a
											href="schoolDocInfo!schoolDocInfo?page=<s:property value="%{pageBean.currentPage-1}"/>" />上一页</a>
									</s:else></td>
								<td><s:if
										test="%{pageBean.currentPage != pageBean.totalPage}">
										<a
											href="schoolDocInfo!schoolDocInfo?page=<s:property value="%{pageBean.currentPage+1}"/>">下一页</a>
										<a
											href="schoolDocInfo!schoolDocInfo?page=<s:property value="pageBean.totalPage"/>">尾页</a>
	...
	<script type="text/javascript">
		...
		function confimdel(obj, str) {
			...
			var url = "delAdminDoctorInfo.action?username=" + str;//处理删除操作
			createXMLHttpRequest();
			xmlHttpReq.open("POST", url, true);
			xmlHttpReq.send(null);
			actionname = "schoolDocInfo!schoolDocInfo?page=";
			xmlHttpReq.onreadystatechange = getResult;
		}
