		...
		<div class="frame-records">
			<div class="records-new part4">
				...
				<div class="context" style="padding-left: 10px !important;">
						...
						<div class="tab-pane active" role="tabpanel" id="detail">
							...
							<table class="table table-hover table-bordered big-data table-striped">
								...
								<tbody>
									<tr>
										<td>档案编号：</td>
										<td id="tableId">${visioncheck.tableId}</td>
										<td>学号：</td>
					...
					<div id="newAss" style="line-height:30px;">
						<label>我的诊断：</label> 
						<label id="isAlready">${assessment}</label>
					</div>
					<div>
						<input class="form-control" type="text" id="assessment" required="required"/>
						<input type="button" class="btn btn-danger" style="width:100px;margin-top:10px;" value="提交" onclick="updateAssess()"/>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	function updateAssess() {
		...
		if (confirm("诊断完毕？"))
			{
				if(document.getElementById("isAlready").innerHTML!="暂无诊断意见"){
					var url = "expertUpdateAssessment.action?tableId=" + tableId+"&assessment="+encodeURI(encodeURI(assessment));
				}
				else{
					var url = "expertInsertAssessment.action?tableId=" + tableId+"&assessment="+encodeURI(encodeURI(assessment));
				}
	...
	function getResult() {
		if (xmlHttpReq.readyState == 4) {
			...
			document.getElementById("isAlready").innerHTML = res.MSG;
		}
	}
