	...
	<div class="content sky-content" id="page-container">
		<div class="frame-records">
			<div class="records-new part4">
				<div class="title" id="school-manage">学校查询
								...
								<s:iterator value="list" id="list" status="st">
									<tr class="gradec">
										...
										<td style="text-align: center;">
											...
											<a href="deleteSchoolSuccess.jsp" class="btn-sm" onclick="del(this)">删除</a>
										</td>
									</tr>
								</s:iterator>
							</tbody>
						...
						<table id="pageTable">
							<tr>
								<td><s:if test="%{pageBean.currentPage == 1}">首页      上一页</s:if>
									<s:else>
										<a href="searchSchoolInfo!searchSchoolInfo?page=1">首页</a>
										<a
											href="searchSchoolInfo!searchSchoolInfo?page=<s:property value="%{pageBean.currentPage-1}"/>" />上一页</a>
									</s:else></td>
								<td><s:if
										test="%{pageBean.currentPage != pageBean.totalPage}">
										<a
											href="searchSchoolInfo!searchSchoolInfo?page=<s:property value="%{pageBean.currentPage+1}"/>">下一页</a>
										<a
											href="searchSchoolInfo!searchSchoolInfo?page=<s:property value="pageBean.totalPage"/>">尾页</a>
									</s:if> <s:else>下一页  尾页</s:else>
								</td>
								<td>页次 <s:property value="pageBean.currentPage" /> / <s:property
										value="pageBean.totalPage" /> 共 <s:property
										value="pageBean.allRow" /> 条记录</td>
							</tr>
						</table>
					</div>
	...
	<script type="text/javascript">
		...
		function getResult() {
			if (xmlHttpReq.readyState == 4) {
				...
				var sch = eval(schoolList.SCHRESULT);
				...
				for ( var i = 0; i < sch.length; i++) {
					...
					Schresult += "<td>" + sch[i].addressid + "</td>";
